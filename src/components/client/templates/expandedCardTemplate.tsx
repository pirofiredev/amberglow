...
export default function expandedCardTemplate({expandedPostData}: {expandedPostData: expandedPostInterface}) {
    ...
    function copyToClipboard() {
        ...
        if(!copied) {
            navigator.clipboard.writeText(expandedPostData.url);
            setCopied(true);
        }
    ...
    return (
        <>
            ...
            <div className={"expandedDescription mt-2 p-2 bg-(--card) border-(--border)"}>
                <p className={"text-(--foreground) rounded-lg text-[0.9rem] line-clamp-3"}>{expandedPostData.description ?? "description unavailable ;<"}</p>
            </div>
            ...
            <div className={"expandedButtons flex flex-row gap-5 mt-3.5 items-center"}>
                ...
                <button onClick={copyToClipboard} className={`toClipboard flex flex-row items-center justify-center gap-1 text-(--muted-foreground) text-sm border-(--border border w-30 px-2 py-1.5 rounded-lg ${copied ? "hover:border-(--muted-foreground) hover:text-(--muted-foreground) cursor-default" : "hover:border-(--primary) hover:text-(--foreground) cursor-pointer"} transition`} type="button">
                    <svg className={copied ? "hidden" : "block"} xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="18" height="18" viewBox="0 0 24 24"><g fill="none" stroke="currentColor" strokeLinecap="round" strokeWidth="2"><path d="m12 17l-1.5 1.5a3.536 3.536 0 0 1-5 0v0a3.536 3.536 0 0 1 0-5l3-3a3.536 3.536 0 0 1 5 0v0"></path><path d="m12 7l1.5-1.5a3.536 3.536 0 0 1 5 0v0a3.536 3.536 0 0 1 0 5l-3 3a3.536 3.536 0 0 1-5 0v0"></path></g></svg>
