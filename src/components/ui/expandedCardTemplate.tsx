...
export default async function expandedCardTemplate() {
    // const postData = await fetchSinglePost()

    return (
        <>
            <div className={"flex flex-row gap-1 text-(--muted-foreground) text-sm mt-7 font-mono"}>
                <p className={"text-(--primary)"}>exampleurl.com</p>
                <p>/</p>
                <p>123 pts</p>
                <p>/</p>
                <p>12h ago</p>
            ...
            <p className={"truncate text-(--foreground) text-lg mt-1.5 max-w-8/10"}>Lorem ipsum is a dummy or placeholder text commonly used in graphic design</p>
            ...
            <div className={"expandedDescription mt-2"}>
                <p className={"text-(--foreground) bg-(--card) border-(--border) border p-3 rounded-lg text-sm mt-1.5"}>Lorem ipsum is a dummy or placeholder text commonly used in commonly used in graphic design</p>
            </div>

            <div className={"expandedButtons flex flex-row gap-5 mt-3.5 items-center"}>

                <a href={"https://google.com"} target={"_blank"} className={"externalUrl flex flex-row gap-1 items-center select-none hover:[&_p]:font-bold transition duration-300 text-(--foreground) text-sm bg-(--primary) px-4 py-1.5 rounded-lg a:hover:font-bold"} rel="noopener">
                    <p>visit article</p>
                ...
                <button className={"toClipboard flex flex-row items-center gap-1 cursor-pointer text-(--muted-foreground) text-sm border-(--border) border px-2 py-1.5 rounded-lg hover:border-(--primary) hover:text-(--foreground) transition"} type="button">
